...
module.exports = function (grunt) {
    ...
    var author    = grunt.option('author') || 'bondrogeen';
    var email     = grunt.option('email')  || 'bondrogeen@gmail.com';
    var fs        = require('fs');
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new apiai name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/apiai.png')) {
            fs.renameSync(__dirname + '/admin/apiai.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/apiai.html')) {
            fs.renameSync(__dirname + '/widgets/apiai.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/apiai/js/apiai.js')) {
            fs.renameSync(__dirname + '/widgets/apiai/js/apiai.js', __dirname + '/widgets/apiai/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/apiai')) {
            fs.renameSync(__dirname + '/widgets/apiai',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Apiai/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Apiai'
                        },
                        {
                            match: /bondrogeen/g,
                            replacement: author
